...
void* VulkanBuffer::map(const VkDeviceSize size, const VkDeviceSize offset)
{
	...
	m_mappedData = data;
    Logger::print("Mapped buffer (ID:" + std::to_string(m_id) + ") memory with size " + std::to_string(size) + " and offset " + std::to_string(offset), Logger::Levels::DEBUG);
	return data;
}
...
void VulkanBuffer::unmap()
{
    if (!isMemoryMapped())
    {
        Logger::print("Tried to unmap memory for buffer (ID:" + std::to_string(m_id) + "), but memory was not mapped", Logger::Levels::WARN);
        return;
    }
    ...
    Logger::print("Unmapped buffer (ID:" + std::to_string(m_id) + ") memory", Logger::Levels::DEBUG);
...
void VulkanBuffer::setBoundMemory(const MemoryChunk::MemoryBlock& memoryRegion)
{
	...
	Logger::print("Bound memory region to buffer (ID:" + std::to_string(m_id) + ") with size " + std::to_string(m_memoryRegion.size) + " and offset " + std::to_string(m_memoryRegion.offset) + " at chunk " + std::to_string(m_memoryRegion.chunk), Logger::Levels::INFO);
...
void VulkanBuffer::free()
{
	...
	Logger::print("Freed buffer (ID:" + std::to_string(m_id) + ")", Logger::Levels::INFO);
