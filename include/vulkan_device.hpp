...
public:
	...
	ResourceID createPipelineLayout(const std::vector<uint32_t>& p_DescriptorSetLayouts, const std::vector<VkPushConstantRange>& p_PushConstantRanges);
	...
	ResourceID createShader(const std::string& p_Filename, VkShaderStageFlagBits p_Stage, bool p_GetReflection, const std::vector<VulkanShader::MacroDef>& p_Macros);
	...
	ResourceID createDescriptorPool(const std::vector<VkDescriptorPoolSize>& p_PoolSizes, uint32_t p_MaxSets, VkDescriptorPoolCreateFlags p_Flags);
	...
	ResourceID createDescriptorSetLayout(const std::vector<VkDescriptorSetLayoutBinding>& p_Bindings, VkDescriptorSetLayoutCreateFlags p_Flags);
	...
	std::vector<ResourceID> createDescriptorSets(uint32_t p_Pool, uint32_t p_Layout, uint32_t p_Count);
...
private:
	...
	struct ThreadCommandInfo
	{
		...
		std::unordered_map<QueueFamilyIndex, VkCommandPool> commandPools;
	};
	...
	std::unordered_map<ThreadID, ThreadCommandInfo> m_ThreadCommandInfos;
